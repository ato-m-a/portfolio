...
const ListItem: FC<SkillSchema> = ({ id, name, icon, contents }) => {
  ...
  return (
    <li>
      <div
        className="relative px-4 py-4 border border-gray-400 shadow cursor-pointer font-medium"
        onClick={toggle}
      >
        <SkillTitle name={name} icon={icon} />
        <Arrow
          $activate={isActive}
          className="block absolute w-4 h-2 right-4 bg-center bg-no-repeat bg-cover"
        />
      </div>
